...
class WaveDecomposer(PreFilter):
    ...
    def filter(self, data, **kwargs):
        coeffs = self.get_thresholed_coeffs(
            data, self.selected_channel, self.threshold_ratio
        )
    ...
    def get_thresholed_coeffs(
        self, data, selected_channel, threshold_ratio=0.3
    ):
        ...
        for i in range(0, len(coeffs)):
            if i in selected_channel:
                coeffs_thresholded[i] = pywt.threshold(
                    coeffs[i], threshold, mode="soft"
                )
            else:
                coeffs_thresholded[i] = pywt.threshold(
                    coeffs[i], threshold * 1000, mode="hard"
                )
