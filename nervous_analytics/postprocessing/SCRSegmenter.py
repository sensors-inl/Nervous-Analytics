...
class SCRSegmenter(PostFilter):
    ...
    def filter(self, data, **kwargs):
        ...
        onsets, peaks = self._remove_low_elevated_peaks(
            filtered_eda, onsets, peaks
        )
    ...
    def _find_peaks_onsets(self, diff2):
        diff2_thresh = [
            -value if -value > self.diff2_peak_threshold else 0
            for value in diff2
        ]
        peaks = [
            idx
            for idx in range(1, len(diff2_thresh) - 1)
            if diff2_thresh[idx] > diff2_thresh[idx + 1]
            and diff2_thresh[idx] > diff2_thresh[idx - 1]
        ]
    ...
    def _replace_onsets_peaks(self, data, onsets, peaks):
        ...
        for i, onset in enumerate(onsets.copy()):
            ...
            if idx != left and idx != right:
                ...
                onsets[i] = min(
                    max(0, onset - self.onset_offset), len(data) - 1
                )
