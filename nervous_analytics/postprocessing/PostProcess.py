...
class PostProcess(PostFilter):
    """
    PostProcess object is a composition of multiple filters.
    Execute the filters in the order they are passed in the constructor.
    """

    def __init__(self, filters: list[PostFilter], additional_output_name=None):
    ...
    def _filter_tracker(self, tracker: PredictionTracker):
        ...
        for process in self.filters:
            ...
            if process.additional_output_names is not None:
                additionnal_outputs = [[output_name, tracker.get_process_output(output_name)]
