...
def test_loss_binary_weighted_crossentropy():
    loss_fn = get_loss_function('binary_weighted_crossentropy', weight_one=0.7, weight_zero=0.1)
    assert isinstance(loss_fn, Loss)
    assert check_simple_model_load(loss_fn)

def test_loss_binary_balanced_crossentropy():
    loss_fn = get_loss_function('binary_balanced_crossentropy', balance_ratio=0.3)
    assert isinstance(loss_fn, Loss)
    assert check_simple_model_load(loss_fn)
